...
\Card[Example]{Give an example of a coset}{
    $G=$ integers ({\bf Z} and $+$\\
    $H= (m{\bf Z})$, (\{\dots, -3m, -2m, -m, 0, m, 2m, dots\})\\
    Cosets are $(m{\bf Z}), (m{\bf Z})+1, (m{\bf Z})+2, \dots$
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\Card[Theorem 1.6]{Lagrange's theorem}{
    If $G$ is a finite group and $H$ is a subgroup of $G$, then
    $\abs{H}$ divides $\abs{G}$.
}

\Card[Notation]{For group $G$ and subgroup $H$, $G / H$ and $G \backslash H$}{
    $G/H\equiv$ set of left cosets, $\{gH: g\in G\}$\\
    $G\backslash H\equiv$ set of right cosets, $\{Hg: g\in G\}$
}

\Card[Notation]{$\abs{G : H}$}{
    Suppose $H$ is a subgroup of $G$.  The number of distinct right
    cosets of $H$ in $G$ is written as $\abs{G : H}$.
}

\Card{index}{
    Suppose $H$ is a subgroup of $G$.  If $G$ is finite, then
  \[
    \abs{G : H} =\abs{G}/\abs{H}
  \]
