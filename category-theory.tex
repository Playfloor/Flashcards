...
\Card{$Hom_{\bf \rm set}(X,Y)$}{The set of functions $X\to Y$}
...
\Card[Notation]{$Y^X$}{$Y^X\equiv {\rm Hom_{Set}(X, Y)}$.}

\Card[Proposition]{Currying}{
    For any sets $A, X, Y$, $\exists$ a bijection
$$C: {\rm Hom_{Set}(X\times A, Y)} \to {\rm Hom_{Set}(X, {\rm Hom_{Set}(A, Y)})}$$
i.e.,
...
Y)\backslash ~$ where $z\sim f(z)$ and $z\sim g(z)$. So,
...
\Card{Hom set of $A$, $B$}{The set of mappings between $A$ and $B$}
